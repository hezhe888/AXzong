...
MAPPING_FILE = os.path.join(SCRIPT_DIR, 'shared', 'pub_mapping.json')
def _load_webhooks():
    urls = []
    # Primary webhook
...
def load_jk_mids():
    if not os.path.exists(MAPPING_FILE):
        raise FileNotFoundError(f"pub_mapping.json not found at {MAPPING_FILE}")
    with open(MAPPING_FILE, 'r', encoding='utf-8') as f:
        mapping = json.load(f)
    jk_mids = [k for k, v in mapping.items() if 'jk' in v.lower()]
    if not jk_mids:
        raise ValueError("No JK pubs found in mapping")
    return jk_mids
