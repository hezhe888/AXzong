...
## 代码质量（铁律）
- 每次修改 `index.html` 后，提交前必须自动验证：
  1. HTML 标签配对（`<div>` vs `</div>` 数量一致）
  2. JS 括号配对（`{` vs `}` 数量一致）
  3. 语义模式检查：
     - 搜索 `);}` 是否出现在隐式 return（`=> (`）回调的关闭位置
     - 搜索无匹配的块体关闭模式（`^\s*\);\s*$` 紧跟 `^\s*\}\s*\n\s*\)\}`）
  4. 验证通过方可 commit
- 若验证失败，定位并修复后再提交，绝不 commit 有语法错误的代码
- **编辑前**：对 `oldString` 先用 grep 确认只匹配**唯一 1 处**，若匹配到多处则扩写上下文使定位唯一
- **commit 前**：除静态验证外，还需抓取实际服务返回的 HTML，搜索已知 Babel 致命模式（`\);}` 非法闭合等），确保无误
